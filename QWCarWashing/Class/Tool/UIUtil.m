...
+ (UIView *)drawLineInView:(UIView *)view frame:(CGRect)frame {
    UIView *line = [self getLineWithFrame:frame color:[UIColor colorWithHexString:@"0xf6f6f6" ]];
    [view addSubview:line];
    return line;
}
...
+ (UIButton *)drawDefaultButton:(UIView *)view title:(NSString *)title target:(id)target action:(SEL)action {
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    btn.frame = CGRectMake((QWScreenWidth-300)/2, 0, QWScreenWidth-20, 40);
...
//    [btn setBackgroundImage:[UIImage createImageWithColor:[UIColor colorWithHexString:@"0xFFB500"]] forState:UIControlStateNormal];
