...
+ (void)post:(NSDictionary *)param andurl:(NSString *)url success:(SuccessBlock)success  fail:(AFNErrorBlock)fail
{
    //MD5加密
    NSDictionary *params = @{
                             @"JsonData" : [NSString stringWithFormat:@"%@",[AFNetworkingTool convertToJsonData:param]],
                             @"Sign" : [NSString stringWithFormat:@"%@",[LCMD5Tool md5:[AFNetworkingTool convertToJsonData:param]]]
                             };
    // 创建请求类
    AFHTTPSessionManager *manager = [self manager];
